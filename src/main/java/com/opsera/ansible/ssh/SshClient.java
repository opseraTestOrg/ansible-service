...
/**
 * Ssh Client used to connect to server instance and execute command. The
 * build-in implementation is {@link SshClientSSHJ}
 * <p>
 * 
 * Client can be used in chain mode,
 * {@link SshClient}.{@link #init(SshClientConfig)}.{@link #connect(int)}.{@link #authPassword()}.{@link #startSession(boolean)}.{@link #executeCommand(String, int)}
 * <p>
 * 
 * At last, close the client with {@link #disconnect()}
 * 
 * <p>
 * Set an {@link SshClientEventListener} with
 * {@link #setEventListener(SshClientEventListener)} to be notified when its
 * event occurs
 * <p>
 * 
 * @author sreeni
...
public interface SshClient {
     ...
     * @return SshClient itself
     * @throws SshException if server is unreachable, usually the host and port is
     *                      incorrect
     */
     ...
     * start session
     * 
     * @param shellMode <tt>true</tt>: communicate with server interactively in
     *                  session, just like command line
     *                  <p>
     *                  <tt>false</tt>: only execute command once in session
     * @return SshClient itself
    ...
    /**
     * 
     * @param command          execute the {@code command} on server instance, and
     *                         timeout if longer than {@code timeoutInSeconds}.
     * @param timeoutInSeconds timeout in seconds
     * @return SshResponse
     * 
     */
     ...
     *         <p>
     *         <tt>inited</tt> before {@link #startSession(boolean)} success
     *         <p>
     *         <tt>connected</tt> after {@link #startSession(boolean)} success
     *         <p>
     *         <tt>disconnected</tt> after {@link #disconnect()}, or any connection
     *         problem occurs
     */
