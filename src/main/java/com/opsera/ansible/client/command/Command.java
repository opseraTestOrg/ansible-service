...
import com.opsera.ansible.util.JsonUtil;
...
public abstract class Command {
    ...
    public String getExecutable() {
        return AnsibleClientConstants.ANSIBLE_MSG;
    }
    ...
    public Map<String, ReturnValue> parseCommandReturnValues(List<String> rawOutput) {
        ...
        for (String key : returnValues.keySet()) {
            ...
            if (returnValue.getResult() == ReturnValue.Result.unmanaged) {
                returnValue.getStdout().add(AnsibleClientConstants.WARNING_COULD_NOT_MATCH_SUPPLIED_HOST_PATTERN_IGNORING + key);
            } else {
                try {
